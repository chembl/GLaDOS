...
def get_label_from_property_name(es_doc_type, prop_name):
    entity_name = es_doc_type
    prop_parts = prop_name.split('.')
    label = ''
    prop_label_id = 'glados_es_gs__' + es_doc_type + '__'
    if len(prop_parts) > 1:
...
def get_js_mapping(prop_name, es_util_mapping, level, es_doc_type):
    ...
    prop_label_id, label, label_mini = get_label_from_property_name(es_doc_type, property_name)
...
def summarize_mapping_definition(es_doc_type, mapping, parent_property=None, level=0):
    properties_summary = []
    for es_property in sorted(mapping.keys()):
        ...
        properties_summary.append(
            '{0} : {1}'.format(
                formatted_prop_name,
                get_js_mapping(property_name, es_property_mapping, level + 2, es_doc_type)
            )
        )
    return properties_summary


def summarize_mappings_types(mappings, glados_schema_file, level=0):
