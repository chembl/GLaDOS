describe 'Columns Handler', ->
  ...
  describe 'With default and additional columns set', ->
  ...
  describe 'With only default columns set', ->
    columnsHandler = undefined

    defaultColumns = TargetRelation.COLUMNS_SETTINGS.ALL_COLUMNS
    ...
    it 'sets all columns list correctly', ->

      allColumns = columnsHandler.get('all_columns')
      comparatorsMustBe = (col.comparator for col in defaultColumns)
      comparatorsGot = (col.comparator for col in allColumns)
